  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled
        ? "bg-surface/80 backdrop-blur-2xl py-3 border-b border-border"
        : "py-5 bg-transparent"
        }`}
    >
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              ...
              className={`text-sm font-medium transition-all duration-300 hover:text-primary relative py-2 ${activeSection === link.href.substring(1)
                ? "text-primary"
                : "text-muted"
                }`}
            >
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden absolute top-full left-0 right-0 glass transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
          }`}
      >
        <div className="container py-4 flex flex-col gap-2">
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={(e) => scrollToSection(e, link.href)}
              className={`px-4 py-3 rounded-lg transition-colors ${activeSection === link.href.substring(1)
